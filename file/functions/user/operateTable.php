...
function preRegistration($urltoken, $email) {
  ...
  $sql = "INSERT INTO pre_user (urltoken, email, date) VALUES (?, ?, now())";
  $stmt = $pdo->prepare($sql);
...
function checkUrltoken($urltoken) {
  ...
  $sql = "SELECT email FROM pre_user WHERE urltoken = ? AND date > now() - interval 24 hour";
  $stmt = $pdo->prepare($sql);
  $stmt->bindValue(1, $urltoken, PDO::PARAM_STR);
  $stmt->execute();
...
function getProvisionalEmail($urltoken) {
  ...
  $sql = "SELECT email FROM pre_user WHERE urltoken = ?";
  $stmt = $pdo->prepare($sql);
  $stmt->bindValue(1, $urltoken, PDO::PARAM_STR);
  $stmt->execute();
  $result = $stmt->fetch();
...
function registration($name, $email, $password) {
  ...
  $sql = "INSERT INTO user (name, email, password, status, created_at, updated_at) VALUES (?, ?, ?, 1, now(), now())";
  $stmt = $pdo->prepare($sql);
...
function deleteProvisionalEmail($urltoken) {
  ...
  $sql = "DELETE FROM pre_user WHERE urltoken = ?";
  $stmt = $pdo->prepare($sql);
  $stmt->bindValue(1, $urltoken, PDO::PARAM_STR);
  $stmt->execute();
}
